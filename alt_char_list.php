...
function alt_char_list_dohook($hookname,$args){
	...
	switch ($hookname){
		case "superuser":
			$allowed = (($session['user']['superuser'] & SU_EDIT_USERS) || get_module_pref('allowed'));
			if ($allowed){
				addnav("Actions");
				addnav("","runmodule.php?module=alt_char_list");
			}
			break;
		case "bioinfo":
			$allowed = (($session['user']['superuser'] & SU_EDIT_USERS) || get_module_pref('allowed'));
			...
			if ($allowed){
				...
				addnav("7?`QAlternate Accounts","runmodule.php?module=alt_char_list&id=$id&ret=".urlencode($args['return_link']));
				tlschema();
			}
			break;
	}
...
function alt_char_list_run(){
				...
				while($row = db_fetch_assoc($result)){
					...
					if (db_num_rows($res_ip) > 0){
						while ($row_ip = db_fetch_assoc($res_ip)){
							output_notl("`7%s `&(%s)`n",$row_ip['name'],$row_ip['login']);
						}
					}else{
						output_notl("%s",$nmf);
					}
					...
					if (db_num_rows($res_id) > 0){
						while ($row_id = db_fetch_assoc($res_id)){
							output_notl("`7%s `&(%s)`n",$row_id['name'],$row_id['login']);
						}
					}else{
						output_notl("%s",$nmf);
					}
					rawoutput("</td><td style='text-align:center;'>");
					$sql_email = "SELECT name, login FROM $ac WHERE emailaddress = '{$row['emailaddress']}' AND emailaddress <> '' AND acctid != {$row['acctid']}";
					$res_email = db_query($sql_email);
					if (db_num_rows($res_email) > 0){
						while ($row_email = db_fetch_assoc($res_email)){
							output_notl("`7%s `&(%s)`n",$row_email['name'],$row_email['login']);
						}
					}else{
						output_notl("%s",$nmf);
					}
	...
	if ($ret <> "") addnav("Return to viewing character","bio.php?char=".httpget('id')."&ret=$ret");
