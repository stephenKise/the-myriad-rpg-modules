...
function lodgegems_dohook($hookname,$args){
	...
	switch ($hookname){
		case "lodge":
			...
			if ($donationsleft >= $cost1) addnav(array("%s gems `@(%s DP)",$gems1,$cost1),"runmodule.php?module=lodgegems&op=cost1&gems=gems1");
			if ($donationsleft >= $cost2) addnav(array("%s gems `@(%s DP)",$gems2,$cost2),"runmodule.php?module=lodgegems&op=cost2&gems=gems2");
			if ($donationsleft >= $cost3) addnav(array("%s gems `@(%s DP)",$gems3,$cost3),"runmodule.php?module=lodgegems&op=cost3&gems=gems3");
