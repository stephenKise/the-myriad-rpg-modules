...
function lodgegold_dohook($hookname,$args){
	...
	switch ($hookname){
		case "lodge":
			...
			if ($donationsleft >= $cost1) addnav(array("%s Gold `@(%s DP)",$gold1,$cost1),"runmodule.php?module=lodgegold&op=cost1&gold=gold1");
			if ($donationsleft >= $cost2) addnav(array("%s Gold `@(%s DP)",$gold2,$cost2),"runmodule.php?module=lodgegold&op=cost2&gold=gold2");
			if ($donationsleft >= $cost3) addnav(array("%s Gold `@(%s DP)",$gold3,$cost3),"runmodule.php?module=lodgegold&op=cost3&gold=gold3");
