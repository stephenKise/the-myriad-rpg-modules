<?php
	...
	$mysqli = new mysqli('localhost', 'root', 'Myr1ad', 'LOTGD');
	...
	if( $cityid > 0 )
	{
		...
		while( list($key,$val) = each($post) )
		{
			if( in_array($key, $field_array) )
			{
				...
				$sql .= "`$key` = '".mysqli_real_escape_string($mysqli, $val)."', ";
		...
		$sql .= ( count($field2) > 0 ) ? "citytext = '".mysqli_real_escape_string($mysqli, serialize($field2))."', " : "citytext = '', ";
		$sql .= ( count($field3) > 0 ) ? "stabletext = '".mysqli_real_escape_string($mysqli, serialize($field3))."', " : "stabletext = '', ";
		$sql .= ( count($field4) > 0 ) ? "armortext = '".mysqli_real_escape_string($mysqli, serialize($field4))."', " : "armortext = '', ";
		$sql .= ( count($field5) > 0 ) ? "weaponstext = '".mysqli_real_escape_string($mysqli, serialize($field5))."', " : "weaponstext = '', ";
		$sql .= ( count($field6) > 0 ) ? "mercenarycamptext = '".mysqli_real_escape_string($mysqli, serialize($field6))."', " : "mercenarycamptext = '', ";
		$sql .= ( count($field7) > 0 ) ? "cityblockmods = '".mysqli_real_escape_string($mysqli, serialize($field7))."', " : "cityblockmods = '', ";
		$sql .= ( count($field8) > 0 ) ? "cityblocknavs = '".mysqli_real_escape_string($mysqli, serialize($field8))."' " : "cityblocknavs = '' ";
	...
	else
	{
		...
		while( list($key,$val) = each($post) )
		{
			...
			{
				array_push($cols,"`$key`");
				array_push($vals,mysqli_real_escape_string($mysqli, $val));
				unset($post[$key]);
			}
			elseif( in_array($key, $field_array2) )
			{
		...
		if( count($field2) > 0 )
		{
			...
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field2)));
		}
		if( count($field3) > 0 )
		{
			array_push($cols, 'stabletext');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field3)));
		}
		if( count($field4) > 0 )
		{
			array_push($cols, 'armortext');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field4)));
		}
		if( count($field5) > 0 )
		{
			array_push($cols, 'weaponstext');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field5)));
		}
		if( count($field6) > 0 )
		{
			array_push($cols, 'mercenarycamptext');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field6)));
		}
		if( count($field7) > 0 )
		{
			array_push($cols, 'cityblockmods');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field7)));
		}
		if( count($field8) > 0 )
		{
			array_push($cols, 'cityblocknavs');
			array_push($vals, mysqli_real_escape_string($mysqli, serialize($field8)));
		}
