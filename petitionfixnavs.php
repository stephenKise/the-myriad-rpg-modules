...
function petitionfixnavs_dohook($hookname, $args){
	...
	case "footer-viewpetition":
		...
		addnav("Nav Ops");
		$id=httpget('id');
		addnav("Fix This Users Navs","runmodule.php?module=petitionfixnavs&op=fixnav&id=$id");
...
function petitionfixnavs_run(){
	...
	switch ($op) {
		...
		case "cfnext":
			...
			invalidatedatacache("petition_counts");
			$user = db_fetch_assoc(db_query("SELECT name FROM ".db_prefix('accounts')." WHERE acctid=$author"));
			require_once("lib/redirect.php");
			redirect("viewpetition.php?op=view&id=$nextid");
		break;
		case "fixnav":
...
function pt_insert($text) {
	...
	injectcommentary("pet-$id","",$text, $schema=false);
	return;
}
?>
